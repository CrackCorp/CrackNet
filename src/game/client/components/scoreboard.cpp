...
void CScoreboard::RenderGoals(float x, float y, float w)
{
	...
	{
		if(m_pClient->m_Snap.m_pGameDataObj->m_ScoreLimit)
		{
			char aBuf[64];
			str_format(aBuf, sizeof(aBuf), "%s: %d", Localize("Score limit"), m_pClient->m_Snap.m_pGameDataObj->m_ScoreLimit);
			TextRender()->Text(0, x+10.0f, y + (h - 20.f) / 2.f, 20.0f, aBuf, -1);
		}
		if(m_pClient->m_Snap.m_pGameDataObj->m_TimeLimit)
		{
			char aBuf[64];
			str_format(aBuf, sizeof(aBuf), Localize("Time limit: %d min"), m_pClient->m_Snap.m_pGameDataObj->m_TimeLimit);
			TextRender()->Text(0, x+230.0f, y + (h - 20.f) / 2.f, 20.0f, aBuf, -1);
		}
		if(m_pClient->m_Snap.m_pGameDataObj->m_RoundNum && m_pClient->m_Snap.m_pGameDataObj->m_RoundCurrent)
		{
			char aBuf[64];
			str_format(aBuf, sizeof(aBuf), "%s %d/%d", Localize("Round"), m_pClient->m_Snap.m_pGameDataObj->m_RoundCurrent, m_pClient->m_Snap.m_pGameDataObj->m_RoundNum);
...
void CScoreboard::RenderScoreboard(float x, float y, float w, int Team, const char *pTitle)
{
	...
	if(m_pClient->m_Snap.m_pGameData->m_GameStateFlags&GAMEFLAG_TEAMS)
	{
	...
	else if(m_pClient->m_GameInfo.m_aTeamSize[Team] > 12)
	{
		LineHeight = 40.0f;
		TeeSizeMod = 0.8f;
		Spacing = 0.0f;
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		*/
		...
		for(int j = i + 1; j < MAX_CLIENTS; j++)
		{
			...
			const CGameClient::CPlayerInfoItem *pInfo2 = &m_pClient->m_Snap.m_aInfoByScore[j];


			if(!pInfo2 || m_pClient->m_aClients[j].m_Team != Team)
				continue;
		...
		if (OldDDTeam == -1)
		{
			for (int j = i - 1; j >= 0; j--)
			{
				const CNetObj_PlayerInfo *pInfo2 = m_pClient->m_Snap.m_paInfoByName[j];

				if(!pInfo2 || m_pClient->m_aClients[j].m_Team != Team)
					continue;

				OldDDTeam = m_pClient->m_Teams.Team(j);
				break;
			}
		}
		...
		if(m_pClient->m_GameInfo.m_TimeScore && g_Config.m_ClDDRaceScoreBoard)
		{
			...
			str_format(aBuf, sizeof(aBuf), "%d", clamp(pInfo->m_pPlayerInfo->m_Score, -999, 99999));
		tw = TextRender()->TextWidth(0, FontSize, aBuf, -1);
		TextRender()->SetCursor(&Cursor, ScoreOffset+ScoreLength-tw, y + (LineHeight - FontSize) / 2.f, FontSize, TEXTFLAG_RENDER);
		TextRender()->TextEx(&Cursor, aBuf, -1);

		// flag
		if(m_pClient->m_Snap.m_pGameData->m_GameStateFlags&GAMEFLAG_FLAGS &&
			m_pClient->m_Snap.m_pGameData && (m_pClient->m_Snap.m_pGameDataFlag->m_FlagCarrierRed == pInfo->m_ClientID ||
			m_pClient->m_Snap.m_pGameDataFlag->m_FlagCarrierBlue == pInfo->m_ClientID))
		{
			...
			RenderTools()->SelectSprite(m_pClient->m_Snap.m_pGameDataFlag->m_FlagCarrierBlue == pInfo->m_ClientID ? SPRITE_FLAG_BLUE : SPRITE_FLAG_RED, SPRITE_FLAG_FLIP_X);
		...
		{
			ColorRGBA rgb = color_cast<ColorRGBA>(ColorHSLA((300.0f - clamp(pInfo->m_pPlayerInfo->m_Latency, 0, 300)) / 1000.0f, 1.0f, 0.5f));
...
void CScoreboard::OnRender()
{
	...
	if(m_pClient->m_Snap.m_pGameData)
	{
		...
		else
		{
			...
			if(m_pClient->m_Snap.m_pGameData->m_GameStateFlags&GAMESTATEFLAG_GAMEOVER && m_pClient->m_Snap.m_pGameData)
			{
				char aText[256];
...
bool CScoreboard::Active()
{
	...
	// if the game is over
	if(m_pClient->m_Snap.m_pGameData && m_pClient->m_Snap.m_pGameData->m_GameStateFlags&GAMESTATEFLAG_GAMEOVER)
		return true;
